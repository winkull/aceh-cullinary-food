                ...
                <div class="col col-lg">
                    ...
                    <div class="card card-custom card-border">
                        <div class="card-header ribbon ribbon-top">
                            <div class="ribbon-target bg-dark" style="top: -2px; right: 20px;">Form Promo Khusus</div>
                            <div class="card-title align-items-start flex-column" style="margin-top: 15px;">
                                ...
                                <span class="text-muted font-weight-bold font-size-sm mt-1">Isilah data berdasarkan pada kolom - kolom tersebut.</span>
                            </div>
                        </div>
                        <!--begin::Form-->
                        <form method="POST" action="{{ route('promokhusus.add') }}" class="form" id="tenant-form" enctype="multipart/form-data">
                            @csrf
                            <div class="card-body">
                                <div class="row">
                                    <div class="col col-lg-6">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="datetime-local" name="awal" class="form-control form-control-solid" value="{{ date('Y-m-d\TH:i', strtotime($data->tanggal)) }}" placeholder="Masukkan Periode Awal" required />
                                        </div>                                        
                                        <div class="form-group">
                                            <label>Periode Akhir</label>
                                            <input type="datetime-local" name="akhir" class="form-control form-control-solid" value="{{ date('Y-m-d\TH:i', strtotime($data->tanggal)) }}" placeholder="Masukkan Periode Akhir" required />
                                        </div>
                                        <div class="form-group">
                                            <label>Nama Pengguna</label><br>
                                            <select class="form-control form-control-solid form-control" id="pengguna" name="pengguna" required>
                                                <option value=""></option>
                                                @foreach ($data->pengguna as $item)
                                                        <option name="pengguna" value="{{$item->idpengguna}}">{{ $item->namapengguna }} || {{$item->emailpengguna}}  </option>
                                                    @endforeach
                                            </select>
                                    ...
                                    <div class="col col-lg-6">
                                        <div class="form-group">
                                            ...
                                            <input type="number" name="nilaipersen" class="form-control form-control-solid" value="{{ old('nilaipersen') }}" placeholder="Masukkan nilai persen" required />
                                        </div>
                                        <div class="form-group">
                                            <label>Nilai Rupiah</label>
                                            <input type="number" name="nilairupiah" class="form-control form-control-solid" value="{{ old('nilairupiah') }}" placeholder="Masukkan nilai rupiah" required />
                                        </div>
                                        <div class="form-group">
                                            <label>Status</label>
